...
def main():
    ...
    ap.add_argument("--show_sweep", action="store_true")
    args = ap.parse_args()
    ...
    if args.target not in df.columns:
        raise ValueError(f"Target '{args.target}' not found in CSV.")
    feats = select_features(df, args.target)
    print(f"[INFO] Using {len(feats)} features: {feats[:8]}{'...' if len(feats)>8 else ''}")
    ...
    def run_epoch(train=True):
        ...
        for i in range(0,len(idx),args.batch):
            ...
            probs.append(torch.softmax(logits,1)[:,1].detach().cpu().numpy())
